...
namespace Honeti
{
    ...
    public static class I18NIdentity
    {
        private static readonly Dictionary<Identity.Roles, string> RoleI18NMap =
            new Dictionary<Identity.Roles, string>()
            {
                { Identity.Roles.Hero, "^hero" },
                { Identity.Roles.Engineer, "^engineer" },
                { Identity.Roles.Infantry, "^standard" },
                { Identity.Roles.Sentinel, "^sentry" },
                { Identity.Roles.BalanceInfantry, "^balanced_standard" },
                { Identity.Roles.AutoSentinel, "^auto_sentry" },
                { Identity.Roles.Outpost, "^outpost" },
                { Identity.Roles.Drone, "^aerial" },
                { Identity.Roles.LaunchingStation, "^dart" },
                { Identity.Roles.Judge, "^judge" }
            };

        private static readonly Dictionary<Identity.Camps, string> CampI18NMap =
            new Dictionary<Identity.Camps, string>()
            {
                { Identity.Camps.Red, "^red_team" },
                { Identity.Camps.Blue, "^blue_team" },
            };

        public static string T(Identity.Roles role) =>
            RoleI18NMap.ContainsKey(role) ? I18N.instance.getValue(RoleI18NMap[role]) : role.ToString();


        public static string T(Identity.Camps camp) =>
            CampI18NMap.ContainsKey(camp) ? I18N.instance.getValue(CampI18NMap[camp]) : "";

        public static string T(Identity id) =>
            T(id.camp) + id.order + T(id.role);

        public static string Describe(this Identity id) => T(id);

        public static string Translate(this Identity.Roles role) => T(role);

        public static string Translate(this Identity.Camps camp) => T(camp);
    }
    ...
    public class I18N : MonoBehaviour
    {
        ...
        /// Default language.
        /// </summary>
        private static LanguageCode _defaultLang = LanguageCode.SCN;
        ...
        /// </summary>
        [SerializeField] private LanguageCode _gameLang = LanguageCode.SCN;
        ...
        public void SetLanguage(Int32 lang)
        {
        ...
        public void setLanguage(LanguageCode langCode)
        {
            ...
            GlobalConfig.LanguageCode = _gameLang;
        ...
        void init()
        {
            ...
            string lang = null;
            if (GlobalConfig.LanguageCode == null)
            {
                ...
                {
                    lang = LanguageCode.SCN.ToString();
                }
            }
            else
            {
                lang = GlobalConfig.LanguageCode.ToString();
            }
