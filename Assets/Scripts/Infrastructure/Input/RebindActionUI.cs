...
namespace Infrastructure.Input
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        [Tooltip("Reference to action that is to be rebound from the UI.")] [SerializeField]
        ...
        [Tooltip("Text label that will receive the current, formatted binding string.")] [SerializeField]
        private TMP_Text m_BindingText;

        [Tooltip("Optional UI that will be shown while a rebind is in progress.")] [SerializeField]
        private ModalWindowManager m_RebindOverlay;

        [Tooltip("Optional text label that will be updated with prompt for user input.")] [SerializeField]
        private TMP_Text m_RebindText;

        [Tooltip(
            "Event that is triggered when the way the binding is display should be updated. This allows displaying "
            + "bindings in custom ways, e.g. using images instead of text.")]
        [SerializeField]
        private UpdateBindingUIEvent m_UpdateBindingUIEvent;

        [Tooltip(
            "Event that is triggered when an interactive rebind is being initiated. This can be used, for example, "
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            if (m_RebindText != null)
            {
                ...
                var text = I18N.instance.getValue("^modal.rebind.waiting",
                    new[] { partName, m_RebindOperation.expectedControlType ?? "" });
        ...
        private void SetButtonConfig(InputButtonConfig config)
        {
            var btnConfigPlaceholder = transform.parent.Find("Button Config").Find("Text").GetComponent<TMP_Text>();
            btnConfigPlaceholder.text =
                I18N.instance.getValue($"^settings.input.buttonConfig.{config.ToString().ToLower()}");
            InputManager.ButtonControlPatterns[actionReference.action.name] = config;
        }

        public void SetButtonConfig(string config)
        {
            if (Enum.TryParse<InputButtonConfig>(config, out var result))
                SetButtonConfig(result);
        }
