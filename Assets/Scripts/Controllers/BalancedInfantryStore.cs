...
namespace Controllers
{
    ...
    /// </summary>
    public class BalancedInfantryStore : RobotStoreBase
    {
        // 发弹
        ...
        private readonly ToggleHelper _animatorHelper = new ToggleHelper();
        ...
        private BalancedGroundChassis _groundChassis;
        private Gun _gun;
        ...
        {
            AsyncUpdateDelta = new WaitForFixedUpdate();
            base.Start();
            if (isServer)
            {
                // 初始化组件
                _groundChassis = new BalancedGroundChassis(gameObject, wheels, yaw);
                ...
                isLeg = id.serial == 1;
                if (isLeg) _animator = gameObject.GetComponent<Animator>();
        ...
        protected override void AsyncUpdate()
        {
            ...
            var reverseAngle = _groundChassis.Update(
                ...
                superBattery, CustomizeManager.Instance().Data(
                    id,
                    CustomizeProperties.Chassis.SuperBattery) * 0.6f, turning
            );
            ...
            float rotZ = localEulerAngles.z;
            float rotY = localEulerAngles.y;
            localEulerAngles = new Vector3(0, rotY, rotZ);
            ...
            Power = _infantryRd.velocity.magnitude * 8f;
        }
        ...
        /// 自动执行动画
        /// </summary>
        private void AutoSetAnimationState()
        {
            //Debug.Log($"volocity x = {_infantryRd.velocity.x}\n" +
            //          $"y = {_infantryRd.velocity.y}"+
            //          $"z = {_infantryRd.velocity.z}" );
            //TODO:根据测量速度选择启用前进动画
        }
        ...
        public override void Receive(IAction action)
        {
            ...
            switch (action.ActionName())
            {
                    ...
                    if (act.Receiver == id && !BeCaught)
                    {
                        if (!magazine)
                        {
                            ...
                            _groundChassis.Move(
                                ...
                                * robotAttributes.MaxChassisPower / 38,
                                robotAttributes.MaxChassisPower / 8,
                                superBattery, turning);
                        }
                        else if (!_supplying)
                        {
                            var robotAttributes = AttributeManager.Instance().RobotAttributes(this);
                ...
                // StateControl按键所控制的有除了移动及云台运动的其他功能，包括开火，弹仓开关，补给,腿轮动画等
                case ActionID.Input.StateControl:
                    ...
                    if (stateControlAction.Receiver == id)
                    {
                        ...
                        if (isLeg)
                        {
                            switch (_animatorHelper.Toggle(stateControlAction.FunctionM))
                            {
                                case ToggleHelper.State.Re:
