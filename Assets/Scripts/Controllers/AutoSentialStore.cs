...
{
    /// <summary>
    /// <c> Infantry</c> 控制自动哨兵 机器人。
    /// <br/>目前仅供云台手自动操作
    /// <br/>可使用的功能：控制移动、云台旋转、小陀螺、弹舱盖、开火、请求补给。
    /// </summary>
    public class AutoSentialStore : RobotStoreBase
    {
        ...
        public override void Receive(IAction action)
        {
            ...
            switch (action.ActionName())
            {
                case ActionID.DroneControl.AutoDriveTarget:
                    ...
                    if ( !BeCaught && auto.Camp == id.camp)
                    {
                        _agent.SetTarget(auto.Pos);
                    }
                    break;
                ...
                //                 * (partyCountDown > 0 ? robotAttributes.MaxChassisPower / 40: robotAttributes.MaxChassisPower / 60),
                //                 (partyCountDown > 0 ? robotAttributes.MaxChassisPower / 4:robotAttributes.MaxChassisPower / 8),
                //                 superBattery);
                //         }
                //         else if(!_supplying)
                //         {
                ...
                //                 * CustomizeManager.Instance().Data(
                //                     id, CustomizeProperties.Chassis.Velocity)
                //                 * (partyCountDown > 0 ? robotAttributes.MaxChassisPower / 20: robotAttributes.MaxChassisPower / 40),
                //                 (partyCountDown > 0 ? robotAttributes.MaxChassisPower / 4:robotAttributes.MaxChassisPower / 8),
                //                 superBattery); 
                //         }
                //         else
                ...
                //         else { switchEndTime = NetworkTime.time; }
                //         
        ...
        private void StartContinuousAimbotSession()
        {
            ...
            _aimbot.StartSession<Armor>(
                ...
                target =>
                {
                    ...
                    var robotStore = armor.GetComponentInParent<StoreBase>();
                    ...
                    if ((armor.transform.position - transform.position).magnitude > 40) return false;
        ...
        protected override void FixedUpdate()
        {
            base.FixedUpdate();
            if (!isServer) return;
            if(!ClockStore.Instance().playing) return;
            if (!_initAddBullet)
            {
            ...
            spinning = _agent.IsReach() ? true : false;
            //单纯的复用isReach
            if (spinning)
            {
                _pitchScanTime += Time.deltaTime;
                var scanUp = _pitchScanTime % 1 > 0.5;
                var scanMultiplier = Vector2.one * 200;
                
                if (!_aimbot.IsTracking())
                {
                    _ptz.View(Time.fixedDeltaTime, Vector2.right, scanMultiplier);
                    _ptz.View(Time.fixedDeltaTime, scanUp ? Vector2.up : Vector2.down, scanMultiplier);
                    StartContinuousAimbotSession();
                    
                }
                else
                {
                    _ptz.View(Time.fixedDeltaTime, _aimbot.Update(Time.fixedDeltaTime), Vector2.one);
                }
